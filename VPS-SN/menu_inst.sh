...
new_banner(){
  ...
  if [[ "${opcion}" ]]; then
    ...
    echo "$opcion" > $local
    [[ ! -e ${VPS_tmp}/message.txt ]] && echo "@Sin_Nombre22" > ${VPS_tmp}/message.txt
    credi="$(less ${VPS_tmp}/message.txt)"
    echo '<h4 style=text-align:center><font color="#00FF40">V</font><font color="#00FFBF">P</font><font color="#00FFFF">S</font><font color="#00FFFF">-</font><font color="#2ECCFA">S</font><font color="#2E9AFE">N</font><font color="#819FF7"></font><br><font color="#819FEB">'$credi'</font></h4>' >> $local
...
fun_autorun () {
  clear
  msg -bar
if [[ $(cat /etc/bash.bashrc | grep -w ${VPS_SN}/menu) ]]; then
  cat /etc/bash.bashrc | grep -v ${VPS_SN}/menu > /tmp/bash
  mv -f /tmp/bash /etc/bash.bashrc
  msg -ama "               $(fun_trans "AUTO-INICIO REMOVIDO")"
  msg -bar
else
  cp /etc/bash.bashrc /tmp/bash
  echo "${VPS_SN}/menu" >> /tmp/bash
  mv -f /tmp/bash /etc/bash.bashrc
...
C_MENU2(){
  unset m_conf
  m_conf="$(cat ${VPS_tmp}/style|grep -w "$1"|awk '{print $2}')"

  case $m_conf in
    0)sed -i "s;$1 0;$1 1;g" ${VPS_tmp}/style;;
    1)sed -i "s;$1 1;$1 0;g" ${VPS_tmp}/style;;
  esac
}

c_stat(){
  unset m_stat
  m_stat="$(cat ${VPS_tmp}/style|grep -w "$1"|awk '{print $2}')"
  case $m_stat in
    0)msg -verm2 "[OFF]";;
    1)msg -verd "[ON]";;
  esac
...
c_resel(){
  ...
  if [[ $txt_r = @(s|S|y|Y) ]]; then
    ...
    read r_txt
    echo -e "$r_txt" > ${VPS_tmp}/message.txt
  fi
  C_MENU2 resel
}
...
echo -e "$(msg -verd " [16]")$(msg -verm2 ">") $(msg -azu "GEN DOMI/CERT-SSL") $([[ -z $(ls "${VPS_crt}") ]] && echo -e "\033[1;31m○ " || echo -e "\033[1;32m◉ ")"
msg -bar
...
case $selection in
  0)return 0;;
  1)${VPS_inst}/dropbear.sh;;
  2)${VPS_inst}/sockspy.sh;;
  3)${VPS_inst}/ssl.sh;;
  4)${VPS_inst}/v2ray.sh;;
  5)${VPS_inst}/ws-cdn.sh;;
  6)${VPS_inst}/budp.sh;;
  7)${VPS_inst}/squid.sh;;
  8)${VPS_inst}/openvpn.sh;;
  9)${VPS_inst}/slowdns.sh;;
  #9)${VPS_inst}/Shadowsocks-libev.sh;;
  #10)${VPS_inst}/shadowsocks.sh;;
  #11)${VPS_inst}/Shadowsocks-R.sh;;
  12)baner_fun;;
  13)cache_ram;;
  14)${VPS_inst}/swapfile.sh;;
  15)${VPS_inst}/ports.sh;;
  16)${VPS_inst}/cert.sh;;
  17)${VPS_inst}/tcp.sh;;
  18)root_pass;;
  19)root_pass 1;;
  20)conf_menu;;
  21)${VPS_SN}/tool_extras.sh;;
  22)fun_autorun;;
esac
}
