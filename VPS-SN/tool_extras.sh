...
install_h(){
  ...
  do
    ...
    if echo -ne $(msg -azu "  Descargando $line3....") && wget -O ${VPS_src}/tool/$line3 https://raw.githubusercontent.com/rudi9999/ADMRufu/main/online/$line3 &>/dev/null; then
      chmod +x ${VPS_src}/tool/$line3
      echo "$line" >> ${VPS_src}/tool/tool
      msg -verd "[ok]"
    else
      msg -verm2 "[fail]"
      rm ${VPS_src}/tool/$line3
    fi
...
if [[ -e "${VPS_src}/tool/$(echo ${script[$selection]}|tr -d '[[:space:]]')" ]]; then
  ${VPS_src}/tool/$(echo ${script[$selection]}|tr -d '[[:space:]]')
