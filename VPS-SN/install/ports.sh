...
edit_squid(){
  ...
  local NEWCONF="$(cat ${CONF}|sed "$line c VPS_port"|sed '/http_port/d')"
  ...
  while read varline; do
    ...
    if [[ ! -z "$(echo "$varline"|grep 'VPS_port')" ]]; then
...
edit_apache(){
  ...
  local NEWCONF="$(cat ${CONF}|sed "$line c VPS-SN")"
  let line++
  ...
  while read varline; do
    ...
    if [[ ! -z "$(echo "$varline"|grep 'VPS-SN')" ]]; then
      for i in `echo $PORT`; do
...
edit_openssh(){
  ...
  local NEWCONF="$(cat ${CONF}|sed "$line c VPS-SN"|sed '/[Pp]ort/d')"
