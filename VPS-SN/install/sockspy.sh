...
while [[ -z $porta_socket ]]; do
 ...
 if [[ $1 = "PDirect" ]]; then
    ...
    if [[ $1 = "PPriv" ]]; then
        ...
        echo "master=NetVPS" > ${VPS_tmp}/pwd.pwd
        while read service; do
            [[ -z $service ]] && break
            echo "127.0.0.1:$(echo $service|cut -d' ' -f2)=$(echo $service|cut -d' ' -f1)" >> ${VPS_tmp}/pwd.pwd
        done <<< "$(mportas)"
         porta_bind="0.0.0.0:$porta_socket"
         pass_file="${VPS_tmp}/pwd.pwd"
         py="python"
    else
        py="python"
    fi
...
echo -e "[Unit]
Description=$1 Service by @Sin_Nombre22
...
ExecStart=/usr/bin/$py ${VPS_inst}/$1.py $conf
Restart=always
RestartSec=3s
