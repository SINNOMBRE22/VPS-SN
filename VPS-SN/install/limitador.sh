...
sshmonitor(){
  h=1
  unlimit=$(cat /etc/VPS-SN/user/unlimit)
    for i in `echo "$user_type"`; do
        ...
        if [[ "$conex" -gt "$s2ssh" ]]; then
        	...
        	usermod -L $user
        	echo "$user $(printf '%(%H:%M:%S)T') $conex/$s2ssh" >> /etc/VPS-SN/user/limit.log
          [[ $unlimit -le 0 ]] && continue || at now +${unlimit} minutes <<< "usermod -U $user" &>/dev/null
        fi
      done
      touch /etc/VPS-SN/user/limit
      timer=$(cat /etc/VPS-SN/user/limit)
      [[ -z ${timer} ]] && timer="3"
      at now +${timer} minutes <<< "/etc/VPS-SN/install/limitador.sh" &>/dev/null
      [[ -z $(cat "/var/spool/cron/crontabs/root"|grep "limitador.sh") ]] && echo "@reboot root /etc/VPS-SN/install/limitador.sh" >> /var/spool/cron/crontabs/root
}
    ...
    for i in `echo "$user_type"`; do
        ...
        if [[ $(date '+%s') -gt $(date '+%s' -d "$userDate") ]]; then
          if [[ $(passwd --status $user|cut -d ' ' -f2) = "P" ]]; then
            usermod -L $user
            echo "$user $(printf '%(%H:%M:%S)T') expirado" >> /etc/VPS-SN/user/limit.log
          fi    
        fi
      done
}
...
# @daily root /etc/VPS-SN/install/limitador.sh >> /var/spool/cron/crontabs/root
