...
ssl_stunel(){
    ...
    title "INSTALADOR SSL By @Sin_Nombre22"
    ...
    if [[ ! "$(echo "$db"|grep ".crt")" = "" ]]; then
        ...
        read opcion
        if [[ $opcion != @(n|N) ]]; then
            cp ${VPS_crt}/$cert ${VPS_tmp}/stunnel.crt
            cp ${VPS_crt}/$key ${VPS_tmp}/stunnel.key
        fi
    fi

    if [[ $opcion != @(s|S) ]]; then
        openssl genrsa -out ${VPS_tmp}/stunnel.key 2048 > /dev/null 2>&1
        (echo "" ; echo "" ; echo "" ; echo "" ; echo "" ; echo "" ; echo "@cloudflare" )|openssl req -new -key ${VPS_tmp}/stunnel.key -x509 -days 1000 -out ${VPS_tmp}/stunnel.crt > /dev/null 2>&1
    fi
    cat ${VPS_tmp}/stunnel.key ${VPS_tmp}/stunnel.crt > /etc/stunnel/stunnel.pem
    sed -i 's/ENABLED=0/ENABLED=1/g' /etc/default/stunnel4
    service stunnel4 restart > /dev/null 2>&1
    msg -bar
    print_center -verd "INSTALADO CON EXITO"
    msg -bar
    rm -rf ${VPS_tmp}/stunnel.crt > /dev/null 2>&1
    rm -rf ${VPS_tmp}/stunnel.key > /dev/null 2>&1
    sleep 3
    return 1
}
